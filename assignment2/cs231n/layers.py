...
def affine_forward(x, w, b):
    ...
    ###########################################################################
    re = x.reshape(x.shape[0], w.shape[0])
    out = re.dot(w) + b
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        x_ = (x-x_mean)/np.sqrt(x_var+eps)
        out = gamma*x_ + beta
        cache['mean'] = x_mean
        cache['variance'] = x_var
        cache['x_hat'] = x_
        cache['gamma'] = gamma
        cache['beta'] = beta
        cache['eps'] = eps
...
def batchnorm_backward(dout, cache):
    ...
    x = cache['x']
    N = x.shape[0]
    sample_mean, sample_var = cache['mean'], cache['variance']
    x_hat = cache['x_hat']
    gamma, beta, eps = cache['gamma'], cache['beta'], cache['eps']
    dgamma = np.sum(dout*x_hat, axis=0)
    dbeta = np.sum(dout, axis=0)
    dx_hat = dout * gamma
    dsigma = -0.5 * np.power(sample_var + eps, -1.5) * np.sum(dx_hat * (x-sample_mean),axis=0)
...
def batchnorm_backward_alt(dout, cache):
    ...
    var_sqrt = 1. / np.sqrt(sample_var + eps)
    dxs1 = - np.sum( dx_hat * var_sqrt, axis=0, keepdims=True)  / N
    print(x_mean.shape)
    dx1 = dx_hat * var_sqrt + dxs1
    print(dx1.shape)
    
    dxs2 = - np.sum(dx_hat * x_mean, axis=0, keepdims=True) * var_sqrt**3 / N
    #print(dxs2.shape)
    dx2 = dxs2*x_mean
    dx = dx1+dx2
    ###########################################################################
        ...
        for f in range(F):
            ...
            for hh in range(H_):
                for ww in range(W_):
                    ...
                    dx_pad[n, :, hh*stride:hh*stride+HH, ww*stride:ww*stride+WW] += w[f] * dout[n, f, hh, ww]
...
def max_pool_forward_naive(x, pool_param):
    ...
    ###########################################################################
    pool_height, pool_width = pool_param.get("pool_height"), pool_param.get("pool_width")
    ...
    for n in range(N):
        for h in range(H_):
            ...
            for w in range(W_):
                ...
                win = x[n, :, h1:h2, w1:w2]
                out[n, :, h, w] = np.max(win.reshape((C, pool_height*pool_width)), axis=1)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, pool_param)
    ...
    for n in range(N):
        for c in range(C):
            for h in range(H_):
                ...
                for w in range(W_):
                    w1 = w*stride
                    win = x[n, c, h1:h1+pool_height, w1:w1+pool_width]
                    dx[n, c, h1:h1+pool_height, w1:w1+pool_width] = (win==np.max(win))*dout[n, c, h, w]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx
